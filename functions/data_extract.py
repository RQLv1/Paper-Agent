                ...
                for mat_key in experimental_data:
                        ...
                        for i, prc_data in enumerate(precursors.values(), 1):
                            if isinstance(prc_data, dict):
                                row.update({
                                    ...
                                    f'prc{i}_value': self.safe_get(prc_data, ['amount', 1, 'value'], ''),
                                    f'prc{i}_unit': self.safe_get(prc_data, ['amount', 2, 'unit'], '')
                                })
                    ...
                    if isinstance(post_processing_steps, dict):
                        for j, proc_step_data in enumerate(post_processing_steps.values(), 1):
                            if not isinstance(proc_step_data, dict):
                                continue
                            try:
                                row[f'post_proc{j}_step'] = proc_step_data.get('step', '')
                                ...
                                for param_name, param_details in parameters.items():
                                    ...
                                    if isinstance(param_details, list):
                                        if len(param_details) >= 1 and isinstance(param_details[0], dict) and 'value' in param_details[0]:
                                        ...
                                        elif len(param_details) >= 2 and isinstance(param_details[1], dict):
                                    ...
                                    elif isinstance(param_details, (str, int, float)):
                                        value = param_details

                                    if param_idx == 1:
                                        row.update({
                                            f'post_proc{j}_value': value,
                                            f'post_proc{j}_unit': unit
                                        })
